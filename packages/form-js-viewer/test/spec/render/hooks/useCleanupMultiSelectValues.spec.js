...
describe("useCleanupMultiSelectValue", function () {
  ...
  it("should fire onChange when any value is no longer in the options", function () {
    ...
    const { rerender } = render(<TestComponent onChangeSpy={onChangeSpy} options={baseOptions} values={values} />, {
      container: root,
    });

    // when
    rerender(<TestComponent onChangeSpy={onChangeSpy} options={noPlatformOptions} values={values} />, {
      container: root,
    });
  ...
  it("should not fire onChange when all values are still in the options", function () {
    ...
    // when
    rerender(<TestComponent onChangeSpy={onChangeSpy} options={baseOptions} values={values} />, {
      container: root,
    });
...
const noPlatformOptions = [{ value: "camunda-cloud", label: "Camunda Cloud" }];
