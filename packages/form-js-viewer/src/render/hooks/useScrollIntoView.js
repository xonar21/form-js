...
export function useScrollIntoView(targetRef, deps, scrollOptions = null, flagRefs = []) {
  useEffect(() => {
    ...
    const shouldScroll =
      scrollOptions.scrollIfVisible || !(itemRect.top >= containerRect.top && itemRect.bottom <= containerRect.bottom);

    if (!shouldScroll) {
      return;
    }

    const { align = "center", offset = 0, behavior = "auto" } = scrollOptions;
