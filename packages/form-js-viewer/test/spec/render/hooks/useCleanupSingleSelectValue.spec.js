...
describe("useCleanupSingleSelectValue", function () {
  ...
  it("should fire onChange when the value is no longer in the options", function () {
    ...
    const { rerender } = render(<TestComponent onChangeSpy={onChangeSpy} options={baseOptions} value={value} />, {
      container: root,
    });

    // when
    rerender(<TestComponent onChangeSpy={onChangeSpy} options={noPlatformOptions} value={value} />, {
      container: root,
    });
  ...
  it("should not fire onChange when the value is still in the options", function () {
    ...
    // when
    rerender(<TestComponent onChangeSpy={onChangeSpy} options={baseOptions} value={value} />, {
      container: root,
    });
